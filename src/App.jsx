...
/* ============================================================
   소규모주민숙원사업 종합검토보고서 v7.7 (빌드 테스트 완료)
   
   주요 수정:
   - EditTable을 memo() 외부 컴포넌트로 분리 → input focus 유지
   - 모든 셀 항상 편집 가능 (editMode 개념 제거)
   - 단가ID 입력 시 품명/규격/단위 자동채움
   - JSON 불러오기 → items state 완전 교체
   - 새로운작업 버튼 / AI분석 버튼
   - 엑셀 3종: xlsx-js-style (2단헤더+색상+수식)
   - 수량산출서: 자재구분 없음, 구체적 산출근거
...
/* 산출근거 DB */
const CALC_BASIS = {
  1:"표토제거 = 폭2.5m × 길이14m = 35m²",
  2:"토사굴착 = 폭2.0m × 길이10m × 깊이2.0m = 40m³",
  3:"기초터파기 = 폭2.5m × 길이20.5m × 깊이1.0m = 51m³",
  4:"잡석기초 = 폭2.5m × 길이20.5m × 두께0.5m = 26m³",
  5:"기초콘크리트 = 폭2.5m × 길이20.5m × 깊이1.0m = 51m³",
  6:"석축찰쌓기 = 높이2.5m × 길이20.5m = 51m²",
  7:"거푸집 = 기초양측면(1.0m×2×20.5m)+전면 = 41m²",
  8:"철근 = HD13@200, 기초체적51m³×배근율 ≒ 2.6ton",
  9:"양생면적 = 기초상면+측면 = 92m²",
  10:"부직포 = 잡석하부+석축배면 ≒ 72m²",
  11:"비닐 = 기초하부 2.5m×20.5m = 51m²",
  12:"뒤채움 = 석축배면 폭1.5m×높이2.5m×길이20.5m×다짐률0.45 ≒ 35m³",
  13:"되메우기 = 잔여 폭1.0m×깊이1.0m×길이20.5m ≒ 20m³",
  14:"사면녹화 = 높이3.0m×길이15m = 45m²",
  15:"사토운반 = 터파기51+토사40+잔해217 = 308m³",
  16:"교통통제 = 공사기간 5일",
  17:"물푸기 = 24시간(터파기 중 용수처리)",
  18:"아스팔트 = 폭2.0m×길이20m = 40m²",
};
...
/* ============================================================
   EditTable — memo()로 분리하여 input focus 유지
   ============================================================ */
const EditTable = memo(function EditTable({ items, onToggleAll, onToggle, onUpdate, allChecked, someChecked }) {
  return (
          ...
          <tr className="bg-slate-700 text-white">
            <th className="border border-slate-600 px-1 py-1.5 w-8">
              <input type="checkbox" checked={allChecked}
                ...
                onChange={onToggleAll} className="w-4 h-4" />
            </th>
            ...
            return (
              <tr key={d.id} className={!d.enabled ? "bg-slate-100 opacity-40 line-through" : isNew ? "bg-yellow-50" : "bg-white hover:bg-blue-50"}>
                <td className="border px-1 py-1 text-center">
                  <input type="checkbox" checked={d.enabled} onChange={() => onToggle(d.id)} className="w-4 h-4" />
                </td>
                <td className="border px-1 py-1 text-center">
                  <input
                    value={d.priceId}
                    onChange={e => onUpdate(d.id, "priceId", e.target.value)}
                    className="w-full text-center bg-transparent text-blue-600 font-medium text-xs outline-none focus:bg-blue-50 focus:ring-1 focus:ring-blue-300 rounded px-1 py-0.5"
                    placeholder="#.번호"
                  />
                </td>
                <td className="border px-1 py-1 text-center">
                  <select value={d.cat} onChange={e => onUpdate(d.id, "cat", e.target.value)}
                    className="text-xs bg-transparent outline-none w-full focus:bg-blue-50">
                ...
                <td className="border px-1 py-1">
                  <input
                    ...
                    className="w-full bg-transparent text-xs outline-none focus:bg-blue-50 focus:ring-1 focus:ring-blue-300 rounded px-1 py-0.5"
                    placeholder="공종명 입력"
                  />
                </td>
                <td className="border px-1 py-1">
                  <input
                    value={d.spec}
                    onChange={e => onUpdate(d.id, "spec", e.target.value)}
                    className="w-full bg-transparent text-xs text-slate-500 outline-none focus:bg-blue-50 focus:ring-1 focus:ring-blue-300 rounded px-1 py-0.5"
                    placeholder="규격 입력"
                  />
                </td>
                <td className="border px-1 py-1 text-center">
                  <input
                    type="number" value={d.qty} step="0.1"
                    onChange={e => onUpdate(d.id, "qty", e.target.value)}
                    className="w-full text-center bg-transparent text-xs font-medium outline-none focus:bg-blue-50 focus:ring-1 focus:ring-blue-300 rounded px-1 py-0.5"
                  />
                </td>
                <td className="border px-1 py-1 text-center">
                  <select value={d.unit} onChange={e => onUpdate(d.id, "unit", e.target.value)}
                    className="text-xs bg-transparent outline-none focus:bg-blue-50">
                    {UNITS.map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </td>
                <td className="border px-1 py-1 text-right text-xs">{fmt(p.total)}</td>
                <td className="border px-1 py-1 text-right text-xs font-medium">
                  {d.enabled ? fmt(Math.round(d.qty * p.total)) : "-"}
                </td>
              </tr>
            );
          })}
...
const loadXLSX = () => new Promise((res, rej) => {
  ...
  s.onerror = () => {
    ...
    s2.onerror = () => rej(new Error("CDN fail"));
...
async function genDesignXL(items, sagub, gwangub, fr) {
  try {
    const X = await loadXLSX(), wb = X.utils.book_new(), act = items.filter(i => i.enabled), ws = {};
    ws["!cols"] = [{wch:8},{wch:24},{wch:28},{wch:10},{wch:6},{wch:12},{wch:14},{wch:12},{wch:14},{wch:12},{wch:14},{wch:12},{wch:14}];
    wsc(ws, 0, 0, "설 계 내 역 서 (소규모주민숙원사업)", {font:{bold:true,sz:14},alignment:{horizontal:"center"}});
    ws["!merges"] = [{s:{r:0,c:0},e:{r:0,c:12}}];
    wsc(ws, 1, 0, "단가근거: 2025년 충청북도 일위대가 목록표", {font:{sz:9,color:{rgb:"666666"}},alignment:{horizontal:"center"}});
    ws["!merges"].push({s:{r:1,c:0},e:{r:1,c:12}});
    ["공종","품 명","규 격","수량","단위","합 계","","노 무 비","","재 료 비","","경 비",""].forEach((v, c) => wsc(ws, 2, c, v, HS));
    ["","","","","","단가","금액","단가","금액","단가","금액","단가","금액"].forEach((v, c) => { if (c >= 5) wsc(ws, 3, c, v, HS); });
    for (let c = 0; c < 5; c++) ws["!merges"].push({s:{r:2,c},e:{r:3,c}});
    ws["!merges"].push({s:{r:2,c:5},e:{r:2,c:6}},{s:{r:2,c:7},e:{r:2,c:8}},{s:{r:2,c:9},e:{r:2,c:10}},{s:{r:2,c:11},e:{r:2,c:12}});
    ...
    CATS.forEach(cc => {
      ...
      if (!ci.length) return;
      const cr = r; r++;
      catRows[cc] = {row: cr, items: []};
      wsc(ws, cr, 0, cc, CS("DBEAFE")); wsc(ws, cr, 1, CAT_NAMES[cc], CS("DBEAFE"));
      ...
      ci.forEach(item => {
        ...
        wsc(ws, ir, 0, "", TS); wsc(ws, ir, 1, item.name, TS); wsc(ws, ir, 2, item.spec || "", TS);
        wsc(ws, ir, 3, item.qty, {...NS, numFmt: "#,##0.0##"});
        wsc(ws, ir, 4, item.unit, {...TS, alignment:{horizontal:"center"}});
        wsc(ws, ir, 5, `=H${ir+1}+J${ir+1}+L${ir+1}`, NS);
        wsc(ws, ir, 6, `=D${ir+1}*F${ir+1}`, NS);
        wsc(ws, ir, 7, p.labor, NS); wsc(ws, ir, 8, `=D${ir+1}*H${ir+1}`, NS);
        wsc(ws, ir, 9, p.material, NS); wsc(ws, ir, 10, `=D${ir+1}*J${ir+1}`, NS);
        wsc(ws, ir, 11, p.expense, NS); wsc(ws, ir, 12, `=D${ir+1}*L${ir+1}`, NS);
      });
      ...
      if (irs.length) {
        [6,8,10,12].forEach(c => {
          ...
          wsc(ws, cr, c, `=SUM(${col}${irs[0]+1}:${col}${irs[irs.length-1]+1})`, {...CS("DBEAFE"), numFmt:"#,##0"});
        });
      }
    });

    // 순공사비
    const sunS = TTS("D6DCE4");
    wsc(ws, sunRow, 0, "", sunS); wsc(ws, sunRow, 1, "순 공 사 비", sunS);
    for (let c = 2; c < 13; c++) wsc(ws, sunRow, c, "", sunS);
    ws["!merges"].push({s:{r:sunRow,c:0},e:{r:sunRow,c:1}});
    const usedCats = CATS.filter(c => catRows[c]);
    [6,8,10,12].forEach(c => {
      const col = String.fromCharCode(65 + c);
      const refs = usedCats.map(cc => `${col}${catRows[cc].row + 1}`).join("+");
      if (refs) wsc(ws, sunRow, c, `=${refs}`, {...sunS, numFmt:"#,##0"});
    });

    // 사급
    const sagubRow = r; r++;
    wsc(ws, sagubRow, 0, "5.", CS("FFF7ED")); wsc(ws, sagubRow, 1, "사급자재대", CS("FFF7ED"));
    for (let c = 2; c < 13; c++) wsc(ws, sagubRow, c, "", CS("FFF7ED"));
    const sagubIrs = [];
    sagub.forEach(item => {
      const ir = r; r++; sagubIrs.push(ir);
      wsc(ws, ir, 0, "", TS); wsc(ws, ir, 1, item.name, TS); wsc(ws, ir, 2, item.spec, TS);
      wsc(ws, ir, 3, item.qty, NS); wsc(ws, ir, 4, item.unit, {...TS, alignment:{horizontal:"center"}});
      wsc(ws, ir, 5, item.unitPrice, NS); wsc(ws, ir, 6, `=D${ir+1}*F${ir+1}`, NS);
      for (let c = 7; c < 13; c++) wsc(ws, ir, c, "", TS);
    });
    if (sagubIrs.length) wsc(ws, sagubRow, 6, `=SUM(G${sagubIrs[0]+1}:G${sagubIrs[sagubIrs.length-1]+1})`, {...CS("FFF7ED"), numFmt:"#,##0"});

    // 관급
    const gwRow = r; r++;
    wsc(ws, gwRow, 0, "6.", CS("FEF2F2")); wsc(ws, gwRow, 1, "관급자재대", CS("FEF2F2"));
    for (let c = 2; c < 13; c++) wsc(ws, gwRow, c, "", CS("FEF2F2"));
    const gwIrs = [];
    gwangub.forEach(item => {
      const ir = r; r++; gwIrs.push(ir);
      wsc(ws, ir, 0, item.sub || "", TS); wsc(ws, ir, 1, item.name, TS); wsc(ws, ir, 2, item.spec, TS);
      wsc(ws, ir, 3, item.qty, NS); wsc(ws, ir, 4, item.unit, {...TS, alignment:{horizontal:"center"}});
      wsc(ws, ir, 5, item.unitPrice, NS); wsc(ws, ir, 6, `=D${ir+1}*F${ir+1}`, NS);
      for (let c = 7; c < 13; c++) wsc(ws, ir, c, "", TS);
    });
    if (gwIrs.length) wsc(ws, gwRow, 6, `=SUM(G${gwIrs[0]+1}:G${gwIrs[gwIrs.length-1]+1})`, {...CS("FEF2F2"), numFmt:"#,##0"});

    // 관급수수료
    const feeRow = r; r++;
    wsc(ws, feeRow, 0, "", CS("FEF2F2")); wsc(ws, feeRow, 1, "관급수수료 (1.5%)", CS("FEF2F2"));
    for (let c = 2; c < 13; c++) wsc(ws, feeRow, c, "", CS("FEF2F2"));
    wsc(ws, feeRow, 6, `=ROUND(G${gwRow+1}*0.015,0)`, {...CS("FEF2F2"), numFmt:"#,##0"});

    // 총공사비
    const grandRow = r; r++;
    const gS = TTS("1E3A5F", "FFFFFF");
    wsc(ws, grandRow, 0, "", gS); wsc(ws, grandRow, 1, "총 공 사 비", gS);
    for (let c = 2; c < 13; c++) wsc(ws, grandRow, c, "", gS);
    ws["!merges"].push({s:{r:grandRow,c:0},e:{r:grandRow,c:1}});
    wsc(ws, grandRow, 6, `=G${sunRow+1}+G${sagubRow+1}+G${gwRow+1}+G${feeRow+1}`, {...gS, numFmt:"#,##0"});

    ws["!ref"] = X.utils.encode_range({s:{r:0,c:0},e:{r,c:12}});
    X.utils.book_append_sheet(wb, ws, "내역서");
    X.writeFile(wb, `설계내역서_${new Date().toISOString().slice(0,10).replace(/-/g,"")}.xlsx`);
  } catch (e) { alert("엑셀 오류: " + e.message); console.error(e); }
}

async function genQtyXL(items, gwangub, fr) {
  try {
    const X = await loadXLSX(), wb = X.utils.book_new(), act = items.filter(i => i.enabled), ws = {};
    ws["!cols"] = [{wch:5},{wch:22},{wch:28},{wch:7},{wch:10},{wch:65},{wch:15}];
    wsc(ws, 0, 0, "수 량 산 출 서 (소규모주민숙원사업)", {font:{bold:true,sz:14},alignment:{horizontal:"center"}});
    ws["!merges"] = [{s:{r:0,c:0},e:{r:0,c:6}}];
    ["No","공종명","규격","단위","수량","산출근거","비고"].forEach((v, c) => wsc(ws, 1, c, v, HS));

    act.forEach((item, i) => {
      const r = i + 2;
      wsc(ws, r, 0, i + 1, {...TS, alignment:{horizontal:"center"}});
      wsc(ws, r, 1, item.name, TS);
      wsc(ws, r, 2, item.spec || "", TS);
      wsc(ws, r, 3, item.unit, {...TS, alignment:{horizontal:"center"}});
      wsc(ws, r, 4, item.qty, {...NS, numFmt: item.unit === "ton" ? "#,##0.000" : "#,##0"});
      wsc(ws, r, 5, CALC_BASIS[item.id] || `설계수량 = ${item.qty}${item.unit}`, TS);
      wsc(ws, r, 6, item.priceId, {...TS, font:{color:{rgb:"2563EB"}}});
    });

    const gTot = gwangub.reduce((s, i) => s + Math.round(i.qty * i.unitPrice), 0);
    const fee = Math.round(gTot * fr);
    const lr = act.length + 2;
    wsc(ws, lr, 0, act.length + 1, {...TS, alignment:{horizontal:"center"}});
    wsc(ws, lr, 1, "관급수수료", TS);
    wsc(ws, lr, 2, "관급자재비×1.5%", TS);
    wsc(ws, lr, 3, "식", {...TS, alignment:{horizontal:"center"}});
    wsc(ws, lr, 4, 1, NS);
    wsc(ws, lr, 5, `관급자재비 ${fmt(gTot)}원 × 1.5% = ${fmt(fee)}원`, TS);
    wsc(ws, lr, 6, "", TS);

    ws["!ref"] = X.utils.encode_range({s:{r:0,c:0},e:{r:lr,c:6}});
...
async function genUnitXL(items, sagub, gwangub) {
  try {
    const X = await loadXLSX(), wb = X.utils.book_new(), ws = {};
    ws["!cols"] = [{wch:10},{wch:25},{wch:32},{wch:14},{wch:14},{wch:14},{wch:14},{wch:16},{wch:25}];
    wsc(ws, 0, 0, "일위대가 단가목록 (2025년 충청북도)", {font:{bold:true,sz:14},alignment:{horizontal:"center"}});
    ws["!merges"] = [{s:{r:0,c:0},e:{r:0,c:8}}];
    ["단가ID","공종명","규격","합계","노무비","재료비","경비","자재구분","출처"].forEach((v, c) => wsc(ws, 1, c, v, HS));
    ...
    pids.forEach(pid => {
      ...
      wsc(ws, r, 0, pid, {...TS, alignment:{horizontal:"center"}, font:{color:{rgb:"2563EB"}}});
      wsc(ws, r, 1, p.name, TS); wsc(ws, r, 2, p.spec, TS);
      wsc(ws, r, 3, p.total, NS); wsc(ws, r, 4, p.labor, NS); wsc(ws, r, 5, p.material, NS); wsc(ws, r, 6, p.expense, NS);
      wsc(ws, r, 7, mt, mt ? {...TS, font:{color:{rgb: mt.includes("관급") ? "DC2626" : "EA580C"}}} : TS);
      wsc(ws, r, 8, "2025 충북 일위대가", TS);
      r++;
    });

    // 사급/관급 자재 단가 (비표준)
    r++;
    wsc(ws, r, 0, "", TS); wsc(ws, r, 1, "[ 사급·관급 자재 단가 ]", {...TS, font:{bold:true, color:{rgb:"B45309"}}});
    for (let c = 2; c < 9; c++) wsc(ws, r, c, "", TS); r++;
    ["품명","규격","자재단가","","","","","구분","출처"].forEach((v, c) => wsc(ws, r, c, v, {...HS, fill:{fgColor:{rgb:"D97706"}}})); r++;

    sagub.forEach(i => {
      wsc(ws, r, 0, "", TS); wsc(ws, r, 1, i.name, TS); wsc(ws, r, 2, i.spec, TS); wsc(ws, r, 3, i.unitPrice, NS);
      for (let c = 4; c < 7; c++) wsc(ws, r, c, "", TS);
      wsc(ws, r, 7, "사급", {...TS, font:{color:{rgb:"EA580C"}}}); wsc(ws, r, 8, i.source, TS); r++;
    });
    gwangub.forEach(i => {
      wsc(ws, r, 0, "", TS); wsc(ws, r, 1, i.name, TS); wsc(ws, r, 2, i.spec, TS); wsc(ws, r, 3, i.unitPrice, NS);
      for (let c = 4; c < 7; c++) wsc(ws, r, c, "", TS);
      wsc(ws, r, 7, "관급", {...TS, font:{color:{rgb:"DC2626"}}}); wsc(ws, r, 8, i.source, TS); r++;
    });

    ws["!ref"] = X.utils.encode_range({s:{r:0,c:0},e:{r,c:8}});
...
export default function App() {
  ...
  const handleNewWork = useCallback(() => {
    if (!window.confirm("현재 데이터를 초기화합니까?")) return;
    _nextId = 200;
    setItems(makeItems());
    setPhotoUrl(null);
    setAnalyzed(true);
  ...
  const handleAnalyze = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      _nextId = 200;
      setItems(makeItems());
      alert("AI 분석 완료! 설계물량이 자동 입력되었습니다.");
    }, 1500);
  }, [photoUrl]);

  /* 체크박스 */
  const allI = useMemo(() => items.every(d => d.enabled), [items]);
  ...
  const updField = useCallback((id, field, value) => {
    setItems(p => p.map(d => {
      if (d.id !== id) return d;
      const updated = {...d, [field]: field === "qty" ? (Number(value) || 0) : value};
      if (field === "priceId" && PRICE_DB[value]) {
        const pr = PRICE_DB[value];
        updated.name = pr.name;
        updated.spec = pr.spec;
        updated.unit = pr.unit;
      }
      return updated;
    }));
  }, []);
  ...
  const addItem = useCallback(() => {
    setItems(p => [...p, {id: nextId(), cat: "4.", name: "", spec: "", unit: "m²", qty: 1, priceId: "", enabled: true}]);
  }, []);
  const delUnchecked = useCallback(() => {
    if (!window.confirm("체크 해제 항목을 삭제합니까?")) return;
  ...
  const gF = Math.round(gT * FEE_RATE);
  const grand = sunG + sT + gT + gF;

  /* JSON 저장/불러오기 */
  const saveJ = useCallback(() => {
    const data = {v: "7.7", items, comment, at: new Date().toISOString()};
    const blob = new Blob([JSON.stringify(data, null, 2)], {type: "application/json"});
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `소규모주민숙원_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (data.items && Array.isArray(data.items)) {
          ...
          // _nextId를 기존 데이터의 max id보다 크게 설정
          const maxId = Math.max(...data.items.map(d => d.id), 200);
          _nextId = maxId + 1;
          alert(`불러오기 완료! (${data.items.length}개 공종)`);
        } else {
          alert("유효한 데이터가 없습니다");
        }
  ...
  return (
    <div className="min-h-screen bg-white" style={{fontFamily:"'Pretendard','Noto Sans KR',sans-serif"}}>
      {/* 헤더 */}
      <div className="bg-slate-800 text-white py-5 px-4">
        <div className="max-w-7xl mx-auto">
          <p className="text-blue-300 text-xs tracking-widest mb-1">소규모주민숙원사업</p>
          <h1 className="text-2xl font-bold">종합검토보고서 <span className="text-sm font-normal text-slate-400">v7.7</span></h1>
        </div>
      </div>
          ...
          <div className="flex gap-1">
            {[["analysis","AI 분석"],["estimate","설계내역서"]].map(([k, l]) => (
              <button key={k} onClick={() => setView(k)}
                className={`px-3 py-1.5 text-xs rounded font-medium ${view === k ? "bg-blue-600 text-white" : "bg-slate-100 hover:bg-slate-200"}`}>{l}</button>
            ))}
          </div>
          <div className="flex gap-1 flex-wrap">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
        ...
        {view === "analysis" && (<>
          <section>
            ...
            <div className="mt-2 flex gap-3 items-center flex-wrap">
              ...
              <button onClick={handleAnalyze} disabled={analyzing}
                className="px-4 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 disabled:opacity-50 font-medium">
                {analyzing ? "⏳ 분석 중..." : "🤖 AI 분석 실행"}
              </button>
              <input ref={fileRef} type="file" accept="image/*" className="hidden"
                onChange={e => { const f = e.target.files?.[0]; if (f) { setPhotoUrl(URL.createObjectURL(f)); setAnalyzed(false); } }} />
            </div>
            {photoUrl && <img src={photoUrl} alt="" className="mt-3 h-40 rounded border cursor-pointer hover:opacity-80" onClick={() => setPhotoModal(true)} />}
          ...
          {analyzed && (<>
            <section>
              ...
              <div className="mt-3 bg-red-50 border border-red-200 rounded-lg p-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                  <div><span className="text-slate-500 text-xs">품목</span><p className="font-semibold">레미콘,철근,석재,잡석</p></div>
                  <div><span className="text-slate-500 text-xs">관급자재비</span><p className="font-bold text-red-600">{fmt(gT)}원</p></div>
                  <div><span className="text-slate-500 text-xs">수수료율</span><p>1.5%</p></div>
                  <div><span className="text-slate-500 text-xs">관급수수료</span><p className="font-bold text-red-600">{fmt(gF)}원</p></div>
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <p className="text-xs text-blue-600 mt-1 mb-2">체크 해제 → 합계 제외 · 단가ID 입력 시 자동채움 · 수량/공종 변경 → 즉시 재계산</p>
              <EditTable items={items} onToggleAll={toggleAllI} onToggle={toggleI} onUpdate={updField} allChecked={allI} someChecked={someI} />
              <div className="mt-2 text-right text-sm font-bold">
                직접공사비: <span className="text-blue-700">{fmt(sunG)}원</span> · 총공사비: <span className="text-slate-800">{fmt(grand)}원</span>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="mt-3 flex justify-center"><XSec s={STRUCT} /></div>
            </section>
          </>)}
        </>)}
        ...
        {view === "estimate" && (<>
          <section>
            ...
            <p className="text-xs text-blue-600 mt-1 mb-2">단가ID 입력 시 품명/규격/단위 자동반영</p>
            <EditTable items={items} onToggleAll={toggleAllI} onToggle={toggleI} onUpdate={updField} allChecked={allI} someChecked={someI} />
          </section>

          <section>
            <Hd n="2" t="설계 내역서" />
            <p className="text-xs text-slate-500 mt-1 mb-3">2025년 충청북도 일위대가</p>
            <div className="overflow-x-auto border rounded-lg">
              <table className="w-full text-xs border-collapse" style={{minWidth:1100}}>
                <thead>
                  <tr className="bg-blue-700 text-white">
                    ...
                    <th rowSpan={2} className="border border-blue-600 px-1 py-1.5 w-12">수량</th>
                    <th rowSpan={2} className="border border-blue-600 px-1 py-1.5 w-8">단위</th>
                    <th colSpan={2} className="border border-blue-600 text-center">합 계</th>
                    <th colSpan={2} className="border border-blue-600 text-center">노 무 비</th>
                    <th colSpan={2} className="border border-blue-600 text-center">재 료 비</th>
                    <th colSpan={2} className="border border-blue-600 text-center">경 비</th>
                  </tr>
                ...
                <tbody>
                  ...
                  {[{c:"1.",n:"토공",t:t1},{c:"2.",n:"구조물공",t:t2},{c:"3.",n:"포장공",t:t3},{c:"4.",n:"부대공",t:t4}].map(({c, n, t}) => {
                    const ci = act.filter(i => i.cat === c);
                    if (!ci.length) return null;
                    return (
                      <Fragment key={c}>
                        <CR13 code={c} name={n} a={[t.g,t.i,t.k,t.m]} />
                        {ci.map((item, idx) => (
                          <IR13 key={item.id} item={item} p={PRICE_DB[item.priceId] || {total:0,labor:0,material:0,expense:0}} idx={idx} />
                        ))}
                      </Fragment>
                    );
                  })}
                  ...
                  {sagub.map((item, idx) => (
                    <tr key={item.id} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                      ...
                      <td className="border px-1 py-1 text-slate-500">{item.spec}</td>
                      <td className="border px-1 py-1 text-center">{item.qty}</td>
                      <td className="border px-1 py-1 text-center">{item.unit}</td>
                      <td className="border px-1 py-1 text-right">{fmt(item.unitPrice)}</td>
                      <td className="border px-1 py-1 text-right">{fmt(Math.round(item.qty * item.unitPrice))}</td>
                      <td colSpan={6} className="border"></td>
                    </tr>
                  ))}

                  <CR13 code="6." name="관급자재대" a={[gT,0,0,0]} fill="bg-red-50" />
                  {gwangub.map((item, idx) => (
                    <tr key={item.id} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                      <td className="border px-1 py-1 text-center text-xs">{item.sub}</td>
                      <td className="border px-2 py-1">{item.name}</td>
                      <td className="border px-1 py-1 text-slate-500">{item.spec}</td>
                      <td className="border px-1 py-1 text-center">{item.qty}</td>
                      <td className="border px-1 py-1 text-center">{item.unit}</td>
                  ...
                  <tr className="bg-red-50 font-bold">
                    <td colSpan={5} className="border px-3 py-1.5 text-center text-red-700">관급수수료 (1.5%)</td>
                    <td className="border"></td>
                    <td className="border px-1 py-1.5 text-right text-red-700">{fmt(gF)}</td>
                    <td colSpan={6} className="border"></td>
                  </tr>
          ...
          <section>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {[["직접공사비",sunG,"blue"],["사급자재비",sT,"orange"],["관급자재비",gT,"red"],["관급수수료",gF,"pink"],["총공사비",grand,"slate"]].map(([l, v, c], i) => (
                <SCard key={i} l={l} v={v} c={c} bold={i === 4} />
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="mt-3 flex gap-3 flex-wrap">
              {[["d","📊 설계내역서","bg-green-600 hover:bg-green-700"],["q","📋 수량산출서","bg-blue-600 hover:bg-blue-700"],["u","📑 일위대가","bg-purple-600 hover:bg-purple-700"]].map(([k, l, cls]) => (
                <button key={k} disabled={xlLoad === k} onClick={() => handleXL(k)}
                  className={`px-4 py-2 ${cls} text-white text-sm rounded shadow disabled:opacity-50`}>
                  {xlLoad === k ? "생성중..." : l}
                </button>
              ))}
            </div>
            <p className="text-xs text-green-700 mt-2">✅ 2단헤더+색상+수식 포함</p>
          </section>
        </>)}

        <footer className="border-t pt-4 pb-6 text-xs text-slate-400">
          <p>2025년 충청북도 일위대가 · 상세 수량산출 근거는 엑셀 참조</p>
        </footer>
      </div>
    </div>
  );
...
function IC({ t, c, children }) { const cls = c === "blue" ? "bg-blue-50 border-blue-200" : "bg-red-50 border-red-200"; const tcls = c === "blue" ? "text-blue-700" : "text-red-700"; return <div className={`${cls} border rounded-lg p-4`}><h4 className={`font-bold ${tcls} text-sm mb-2`}>{t}</h4><div className="space-y-1.5">{children}</div></div>; }
...
function CR13({ code, name, a, fill }) {
  const bg = fill || "bg-blue-50";
...
function SCard({ l, v, c, bold }) {
  const cls = {blue:"bg-blue-50 border-blue-200 text-blue-700",orange:"bg-orange-50 border-orange-200 text-orange-600",red:"bg-red-50 border-red-200 text-red-600",pink:"bg-pink-50 border-pink-200 text-pink-600",slate:"bg-slate-800 border-slate-700 text-white"}[c];
  return <div className={`rounded-lg border p-3 ${cls}`}><p className="text-xs opacity-75">{l}</p><p className={`${bold ? "text-lg" : "text-sm"} font-bold mt-0.5`}>{fmt(v)}원</p></div>;
}

/* 표준단면도 SVG */
function XSec({ s }) {
  const sc = 80;
  const wH = s.wallH * sc, wT = s.wallT * sc * 3, fW = s.foundW * sc, fD = s.foundD * sc, jT = s.japsukT * sc;
  const ox = 140, gy = 260, fT = gy - fD, wTop = fT - wH;

  return (
    <svg viewBox="0 0 650 380" className="w-full max-w-2xl border rounded-lg bg-slate-50 p-2">
      ...
      <rect x={ox} y={gy} width={fW} height={jT} fill="#d6d3d1" stroke="#78716c" />
      <text x={ox + fW / 2} y={gy + jT / 2 + 4} fontSize="9" fill="#44403c" textAnchor="middle">잡석 T={s.japsukT}m</text>
      <rect x={ox - 5} y={gy + jT} width={fW + 10} height={4} fill="#bfdbfe" stroke="#3b82f6" strokeWidth="0.5" />
      <text x={ox + fW + 12} y={gy + jT + 4} fontSize="8" fill="#3b82f6">부직포</text>
      <rect x={ox + 10} y={fT} width={fW - 20} height={fD} fill="#e5e7eb" stroke="#374151" strokeWidth="1.5" />
      <text x={ox + fW / 2} y={fT + fD / 2 - 5} fontSize="10" fill="#1f2937" textAnchor="middle" fontWeight="bold">기초 콘크리트</text>
      <text x={ox + fW / 2} y={fT + fD / 2 + 10} fontSize="8" fill="#4b5563" textAnchor="middle">{s.conSpec} D={s.foundD}m</text>
      <rect x={ox + 10} y={wTop} width={wT} height={wH} fill="#fbbf24" stroke="#92400e" strokeWidth="1.5" rx="2" />
      {Array.from({length: Math.floor(wH / 15)}).map((_, i) => <line key={i} x1={ox + 12} y1={wTop + 10 + i * 15} x2={ox + 8 + wT} y2={wTop + 10 + i * 15} stroke="#92400e" strokeWidth="0.5" opacity="0.4" />)}
      <text x={ox + 10 + wT / 2} y={wTop - 8} fontSize="9" fill="#92400e" textAnchor="middle" fontWeight="bold">찰쌓기 T={Math.round(s.wallT * 100)}cm</text>
      <rect x={ox + 10 + wT} y={wTop} width={60} height={wH} fill="#fef3c7" stroke="#d97706" strokeDasharray="4,2" />
      <text x={ox + 10 + wT + 30} y={wTop + wH / 2} fontSize="9" fill="#92400e" textAnchor="middle">뒤채움</text>
      <line x1={ox + 10 + wT} y1={wTop} x2={ox + 10 + wT} y2={fT} stroke="#3b82f6" strokeWidth="2" strokeDasharray="6,3" />
      <rect x={ox + 10 + wT + 60} y={wTop - 10} width={180} height={10} fill="#374151" rx="1" />
      <text x={ox + 10 + wT + 150} y={wTop - 16} fontSize="11" fill="#1f2937" textAnchor="middle" fontWeight="bold">도 로</text>
      <text x="60" y={fT + 30} fontSize="12" fill="#2563eb" textAnchor="middle" fontWeight="bold">하 천</text>
      <path d={`M45,${fT + 45} Q60,${fT + 50} 45,${fT + 60} Q60,${fT + 65} 45,${fT + 75}`} fill="none" stroke="#2563eb" strokeWidth="1.5" />
      <line x1={ox - 5} y1={wTop} x2={ox - 5} y2={fT} stroke="#dc2626" strokeWidth="0.8" />
      <line x1={ox - 10} y1={wTop} x2={ox} y2={wTop} stroke="#dc2626" strokeWidth="0.8" />
      <line x1={ox - 10} y1={fT} x2={ox} y2={fT} stroke="#dc2626" strokeWidth="0.8" />
      <text x={ox - 22} y={(wTop + fT) / 2 + 4} fontSize="9" fill="#dc2626" textAnchor="middle" transform={`rotate(-90,${ox - 22},${(wTop + fT) / 2})`}>H={s.wallH}m</text>
      <line x1={ox + fW + 5} y1={fT} x2={ox + fW + 5} y2={gy} stroke="#dc2626" strokeWidth="0.8" />
      <text x={ox + fW + 18} y={(fT + gy) / 2 + 4} fontSize="9" fill="#dc2626">D={s.foundD}m</text>
      <line x1={ox + 10} y1={fT - 8} x2={ox + fW - 10} y2={fT - 8} stroke="#059669" strokeWidth="0.8" />
      <text x={ox + fW / 2} y={fT - 12} fontSize="9" fill="#059669" textAnchor="middle">B={s.foundW}m</text>
      {Array.from({length: 5}).map((_, i) => <circle key={i} cx={ox + 25 + i * 35} cy={fT + fD / 2} r="2.5" fill="#ef4444" stroke="#991b1b" strokeWidth="0.5" />)}
      <text x={ox + fW + 35} y={fT + fD / 2 + 4} fontSize="7" fill="#ef4444">● {s.steelSpec.split(",")[0]}</text>
      ...
      <rect x="470" y="320" width="14" height="9" fill="#fbbf24" stroke="#92400e" strokeWidth="0.5" /><text x="490" y="329" fontSize="8">석축</text>
      <rect x="470" y="333" width="14" height="9" fill="#e5e7eb" stroke="#374151" strokeWidth="0.5" /><text x="490" y="342" fontSize="8">기초콘크리트</text>
      <rect x="470" y="346" width="14" height="9" fill="#d6d3d1" stroke="#78716c" strokeWidth="0.5" /><text x="490" y="355" fontSize="8">잡석기초</text>
    </svg>
  );
}
